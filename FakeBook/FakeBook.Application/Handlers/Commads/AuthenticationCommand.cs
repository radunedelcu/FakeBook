  ...
  public class AuthenticationCommand : IAuthenticationCommand {
    ...
    public async Task<ResponseRegisterModel?> Register(RequestRegisterModel requestRegisterModel) {
      try {
        ...
        var accountEntity =
            new UserEntity() { Name = requestRegisterModel.Name.Trim().ToLower(),
                               Email = requestRegisterModel.Email.Trim().ToLower(),
                               HashPassword = hashPassword,
                               Quote = requestRegisterModel.Quote,
                               ProfilePicture =
                                   UploadProfilePicture(requestRegisterModel.ProfilePicture),
                               CoverImage = UploadCoverImage(requestRegisterModel.CoverImage),
    ...
    private void ComputeHashPassword(string password, out byte[] keyPassword,
                                     out byte[] hashPassword) {
